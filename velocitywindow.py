...
class VelocityTopStrip(wx.Panel):
...
class VelocityWindowStrip(wx.Panel):
    HANDLE_SIZE = 10
    ...
    def OnPaint(self, _):
        ...
        # compute the two key positions
        x_low  = int(self.low  * key_w)
        x_high = int(self.high * key_w)

        # draw a green box from low→high
    ...
    def OnMouseUp(self, evt):
        self.selected = None
        for name, (pid, attr) in self._handle_map.items():
            val = getattr(self, attr)
            self.main.send_parameter_edit(pid, val)
...
class LinkVelocityWindowStrip(wx.Panel):
    HANDLE_SIZE = 10
    ...
    def __init__(self, parent, link_index, main, controls):
        ...
        self._handle_map = {
            ...
            'high':    (34, 'high'),
            'lowfade': (33, 'low_fade'),
            'highfade':(35, 'high_fade'),
        }

        # parameters
        self.low, self.high       = 0, 127
        self.low_fade, self.high_fade = 0, 0
        self.selected             = None
        
        self.link_index = link_index
        
        controls["E4_LINK_VEL_LOW"] = self.low
        controls["E4_LINK_VEL_LOWFADE"] = self.low_fade
        controls["E4_LINK_VEL_HIGH"] = self.high
        controls["E4_LINK_VEL_HIGHFADE"] = self.high_fade
